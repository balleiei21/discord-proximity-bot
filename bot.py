...
# ----------------------------------------------------
# 2. ข้อมูลหมวดหมู่และช่องทั้งหมด
# ----------------------------------------------------
CATEGORIES_DATA = {
    "📌 INFORMATION": {
    ...
    if message.content.strip() == "เริ่มงาน":
        ...
        for cat_name, data in CATEGORIES_DATA.items():
            ...
            if cat_name == "🔐 MANAGEMENT":
                mgmt_channels = [
                    {"name": "꒰👑꒱・ห้อง owner", "roles": [r_owner, r_co_owner], "desc": "Owner / Co-Owner"},
                    {"name": "꒰💼꒱・ห้องบริหาร", "roles": [r_owner, r_co_owner, r_executive, r_manager], "desc": "Owner / Co-Owner / Executive / Manager"},
                    {"name": "꒰📊꒱・รายงานร้าน", "roles": [r_owner, r_co_owner, r_executive], "desc": "Owner / Co-Owner / Executive"},
                    {"name": "꒰⚙️꒱・ตั้งค่าระบบ", "roles": [r_owner, r_co_owner, r_admin], "desc": "Owner / Co-Owner / Admin"}
                ]
        ...
        if report_channel:
            try:
                ...
                await report_channel.send(report_msg_1)
            except Exception as e:
...
# Structure สำหรับรันบน Python 3.13+ และ Railway
async def main():
