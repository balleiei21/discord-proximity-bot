...
# ----------------------------------------------------
# ฟังก์ชัน ป้องกัน Rate Limit (Safe Process)
# ----------------------------------------------------
    ...
    while True:
        try:
            ...
            if e.status == 429:
                ...
                print(f"[Rate Limit] หมวดหมู่ {name} ติดชะลอ รอ {retry_after} วินาที...")
...
async def safe_create_text_channel(category, name, overwrites=None):
    while True:
        try:
            ...
            if e.status == 429:
                retry_after = getattr(e, 'retry_after', 5.0)
                print(f"[Rate Limit] ช่อง {name} ติดชะลอ รอ {retry_after} วินาที...")
                await asyncio.sleep(retry_after + 0.5)
            else:
...
async def safe_create_voice_channel(category, name, overwrites=None):
    while True:
        try:
            ...
            if e.status == 429:
                retry_after = getattr(e, 'retry_after', 5.0)
                print(f"[Rate Limit] ช่องเสียง {name} ติดชะลอ รอ {retry_after} วินาที...")
                await asyncio.sleep(retry_after + 0.5)
            else:
