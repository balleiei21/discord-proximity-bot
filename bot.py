...
async def on_message(message):
    ...
    if message.content.strip() == "เริ่มงาน":
        ...
        guild = message.guild
        await message.channel.send(f"รับทราบครับ **กำลังเนรมิตร้าน {NEW_SERVER_NAME}... 🛍️✨**")
        ...
        for cat_name, data in CATEGORIES_DATA.items():
            ...
            if cat_name == "🔐 MANAGEMENT":
                ...
                for m_ch in mgmt_channels:
                    ...
                    try:
                        await category.create_text_channel(m_ch["name"], overwrites=ch_overwrites)
                        await asyncio.sleep(0.2)
                    except Exception as e:
                        print(f"สร้างช่องบริหาร {m_ch['name']} ไม่สำเร็จ: {e}")
                continue
            ...
            for txt_name in data["text"]:
                try:
                    ...
                    # 🎯 กำหนดช่องส่งรายงานไปที่ "บันทึกงาน"
                    if "บันทึกงาน" in txt_name:
                        report_channel = ch
                    await asyncio.sleep(0.2)
                except Exception as e:
        ...
        if report_channel:
            try:
                ...
                await report_channel.send("เนรมิตร้าน NEXORA SHOP เรียบร้อยแล้วครับ! บันทึกรายงานเข้าห้องทีมงานแล้ว 🛍️✨")
            except Exception as e:
