...
# ----------------------------------------------------
# 2. ข้อมูลหมวดหมู่และช่องทั้งหมด (ตรงตามโครงสร้าง NEXORA SHOP)
# ----------------------------------------------------
CATEGORIES_DATA = {
    "📌 INFORMATION": {
        "text": [
...
# ----------------------------------------------------
# ฟังก์ชันป้องกัน Rate Limit แบบบังคับสร้างให้สำเร็จ
# ----------------------------------------------------
    ...
    while True:
        try:
            ...
            if e.status == 429:
                ...
                print(f"[Rate Limit] หมวดหมู่ {name} ติดขัด รอ {retry_after} วินาที...")
...
async def safe_create_text_channel(category, name, overwrites=None):
    while True:
        try:
            ...
            if e.status == 429:
                retry_after = getattr(e, 'retry_after', 5.0)
                print(f"[Rate Limit] ช่อง {name} ติดขัด รอ {retry_after} วินาที...")
                await asyncio.sleep(retry_after)
            else:
                print(f"ไม่สามารถสร้างช่อง {name} ได้: {e}")
                return None
...
async def safe_create_voice_channel(category, name, overwrites=None):
    while True:
        try:
            ...
            if e.status == 429:
                retry_after = getattr(e, 'retry_after', 5.0)
                print(f"[Rate Limit] ช่องเสียง {name} ติดขัด รอ {retry_after} วินาที...")
                await asyncio.sleep(retry_after)
            else:
...
async def on_message(message):
    ...
    if message.content.strip() == "เริ่มงาน":
        ...
        # ดึงออบเจ็กต์ยศไว้ใช้ตั้งค่า Permission
        ...
        for cat_name, data in CATEGORIES_DATA.items():
            ...
            category = await safe_create_category(guild, cat_name, overwrites=overwrites)
            if not category:
                continue
        ...
        # 6. ส่งรายงานสรุปการสร้างลงในช่อง ꒰📝꒱・บันทึกงาน
        if report_channel:
            try:
                embed_roles = discord.Embed(
                    title="👑 รายงานยศและสิทธิ์ประจำร้าน NEXORA SHOP",
                    description="สรุปรายชื่อยศที่ถูกจัดสร้างขึ้น:",
                ...
                embed_summary = discord.Embed(
                    title="✨ สร้างโครงสร้างเซิร์ฟเวอร์เรียบร้อย 100% ✨",
                    description=f"เนรมิตโครงสร้างร้าน **{NEW_SERVER_NAME}** สำเร็จครบทุกช่องทางแล้วครับ!",
                ...
                print(f"ไม่สามารถส่งรายงานเข้าช่องบันทึกงานได้: {e}")
...
if TOKEN:
    ...
    print("ไม่พบ TOKEN ในระบบ กรุณาตรวจสอบการตั้งค่า Environment Variable")
