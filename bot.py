...
# ----------------------------------------------------
# 2. ข้อมูลหมวดหมู่และช่องทั้งหมด (10 หมวดหมู่)
# ----------------------------------------------------
CATEGORIES_DATA = {
    "꒰🌸꒱ Ⅰ・WELCOME": {
        "text": [
...
async def on_message(message):
    ...
    if message.content.strip() == "เริ่มงาน":
        ...
        for cat_name, data in CATEGORIES_DATA.items():
            ...
            # --- สร้าง Voice Channels (ใช้ category.create_voice_channel โดยตรง) ---
        ...
        if report_channel:
            try:
                ...
                embed_summary.add_field(
                    name="꒰⛏️꒱ Ⅳ・MINECRAFT",
                    value="• `꒰📷꒱・minecraft-galleryೀ` | `꒰🏠꒱・build-showcaseೀ` | `꒰👤꒱・skin-showcaseೀ` | `꒰🗺️꒱・map-showcaseೀ` | `꒰🎬꒱・minecraft-clips&*` | `꒰💡꒱・minecraft-ideasೀ`",
                    inline=False
                )
                embed_summary.add_field(
                    name="꒰📺꒱ Ⅴ・CREATOR ZONE",
