...
ROLES_DATA = [
    ...
    {"name": "🧁・Event Team", "color": discord.Color.from_rgb(221, 160, 221), "permissions": perm_staff, "desc": "ทีมงานจัดกิจกรรม แแจกของรางวัล และสร้างสีสัน"},
...
async def on_message(message):
    ...
    if message.content.strip() == "เริ่มงาน":
        ...
        # 6. ส่งรายงานสรุปผล Embed แบบละเอียดไปที่ห้อง server-management
        if report_channel:
            # --- Embed 1: สรุปข้อมูลบทบาท (Roles) ---
            ...
            # --- Embed 2: สรุปหน้าที่ของแต่ละหมวดหมู่และช่อง (Channels & Categories) ---
            embed_channels = discord.Embed(
                title="🎂 รายงานโครงสร้างหมวดหมู่และช่องใช้งาน (Channels Summary)",
                description="รายละเอียดการใช้งานของแต่ละหมวดหมู่:",
                color=discord.Color.from_rgb(255, 182, 193)
            )

            embed_channels.add_field(
                name="꒰🌸꒱ Ⅰ・WELCOME (อ่านได้อย่างเดียว)",
                value="• `welcome`: ต้อนรับสมาชิกใหม่\n• `rules`: กฎระเบียบเซิร์ฟเวอร์\n• `roles`: เลือกรับยศประจำตัว\n• `announcements`: ประกาศสำคัญ\n• `introductions`: ช่องแนะนำตัว",
                inline=False
            )

            embed_channels.add_field(
                name="꒰🍰꒱ Ⅱ・STRAWBERRY GARDEN (คอมมูนิตี้หลัก)",
                value="• `general-chat`: พูดคุยทั่วไป\n• `strawberry-talk`: คุยสบายๆ ฟีลคาเฟ่\n• `gallery`: แชร์รูปภาพ\n• `game-chat`: พูดคุยเรื่องเกม\n• `music-room`: แชร์เพลง/เพลย์ลิสต์\n• `friend-zone`: หาเพื่อนใหม่",
                inline=False
            )

            embed_channels.add_field(
                name="꒰🎀꒱ Ⅲ・SWEET EVENTS (กิจกรรม)",
                value="• `events`: แจ้งกิจกรรมเซิร์ฟเวอร์\n• `giveaways`: กิจกรรมแจกของ\n• `event-results`: สรุปผลกิจกรรม\n• `event-schedule`: ตารางกิจกรรม",
                inline=False
            )

            embed_channels.add_field(
                name="꒰💌꒱ Ⅳ・SUPPORT (ศูนย์ช่วยเหลือ)",
                value="• `open-ticket`: เปิดตั๋วติดต่อทีมงาน\n• `help-center`: สอบถามข้อสงสัย\n• `contact-staff`: ติดต่อทีมงานโดยตรง",
                inline=False
            )

            embed_channels.add_field(
                name="꒰🧁꒱ Ⅴ・STAFF BACKSTAGE (เฉพาะ Staff)",
                value="• `staff-chat`: พุดคุยทีมงาน\n• `staff-board`: มอบหมายงาน\n• `staff-news`: ประกาศภายใน\n• `member-reports`: รายงานสมาชิก\n• `staff-ideas`: เสนอไอเดีย\n• `server-log`: บันทึกข้อมูลเซิร์ฟ",
                inline=False
            )

            embed_channels.add_field(
                name="꒰👑꒱ Ⅵ・ADMIN ROOM (เฉพาะ Manager / Admin / Owner)",
                value="• `admin-chat`: พูดคุยระดับสูง\n• `server-management`: จัดการระบบ/สรุปงาน\n• `admin-log`: ประวัติการทำงานแอดมิน\n• `server-stats`: สถิติเซิร์ฟเวอร์\n• `serious-reports`: รายงานเรื่องร้ายแรง",
                inline=False
            )

            embed_channels.add_field(
                name="꒰🎧꒱ Ⅶ・VOICE LOUNGE (ห้องพูดคุยด้วยเสียง)",
                value="• `Strawberry Café` / `Whipping Cream` / `Cake Room` / `Sweet Room` / `Gaming Room`: ห้องเสียงสำหรับสมาชิก\n• `🔒・Staff Lounge`: ห้องเสียงเฉพาะทีมงาน",
                inline=False
            )
        ...
        # ⚠️ return ออกเพื่อป้องกันไม่ให้เกิดการส่งข้อความซ้ำ
        return
